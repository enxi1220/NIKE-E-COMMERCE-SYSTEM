//    Document   : Fo.CustRegLog.js
//    Created on : Feb 20, 2022, 9:50:23 PM
//    Author     : vinnie chin

//picture preview on customer registration
function picPreview(input) {
    if (input.files && input.files[0]) {
        var reader = new FileReader();
        reader.onload = function (e) {
            $('#custRegPicPre').attr('src', e.target.result);
        }
...
function checkFileInputExist(ImageURL) {
    if ($('#custRegPic')[0].files.length === 0) {
        return dataURLtoFile(ImageURL, 'testing');
    } else {
        return $('#custRegPic')[0].files[0];
    }
}
...
function validatedCustReg(name, username, mail, password, phone, deliveryAddress) {
    ...
    postMultipart('/Nike_E-Commerce_System/FoCustRegAcc',
            [
                submitData('customerData', customerInput),
                submitData('profileImg', checkFileInputExist(defaultImgUrl))
            ],
            function (success) {
                Swal.fire({
                    position: 'center',
            ...
            function (error) {
                Swal.fire({
                    icon: 'error',
                    title: 'Oops...',
                    html: error.responseText.split('.').join('.<br/><br/>')
                });
            }
    );
}
...
$(document).ready(function () {
    ...
    //customer click register link at login
    ...
    //validate number input only for phone
    $("#custRegPhone").keypress(function (event) {
        validateAlpha(event);
    });

    //customer register account form submit
    $('#custRegAcc').submit(function (event) {
        event.preventDefault();

        var name = $('#custRegName').val();
        var username = $('#custRegUsername').val();
        var mail = $('#custRegMail').val();
        var password = $('#custRegPass').val();
        var confirmPass = $('#custRegPass2').val();
        var phone = $('#custRegPhone').val();
        var addr = $('#address').val();
        var code = $('#addressPostcode').val();
        ...
        if (!name) {
            ...
            errorPrompt(append("your username."));

        } else if (!mail) {
            errorPrompt(append("your email."));

        } else if (!password) {
            errorPrompt(append("your password."));

        } else if (confirmPass !== password) {
            errorPrompt("Confirm password is not same with the password set.");
        ...
        //if one of the address fields is filled, check others 
            ...
            //if all address fields are filled, then concat their value to a full address   
                ...
                validatedCustReg(name, username, mail, password, phone, deliveryAddress);
            }
        } else {
            validatedCustReg(name, username, mail, password, phone, deliveryAddress);
        }
    ...
    $('#custLogin').submit(function (event) {

        event.preventDefault();

        var username = $('#custLoginName').val();
        var password = $('#custLoginPass').val();
        var rememberMe = "";

        if ($("#rmbCust").is(":checked")) {
            rememberMe = 1;
        } else {
            rememberMe = 0;
        }
        ...
        if (!username) {
            ...
            post('/Nike_E-Commerce_System/FoCustLogin', customerInput,
                    function (success) {
                        Swal.fire({
                            position: 'center',
                            icon: 'success',
    ...
    //customer submit email for getting OTP
    $('#forgotPass-form').submit(function (event) {

        event.preventDefault();
        setTimeout(function () {
               $('#email-sending-msg').text("Email Sending...Please wait for a while.");
        }, 500);
        ...
        if (!email) {
            errorPrompt(append("your registered email."));
            setTimeout(function () {
                $('#email-sending-msg').text("Please enter registered email so we can send OTP to you.");
            }, 500);
            ...
            post('/Nike_E-Commerce_System/FoCustResetPass', customerInput,
                    function (success) {
                    ...
                    function (error) {
                        setTimeout(function () {
                            $('#email-sending-msg').text("No email found...Please try again.");
                        }, 500);
    ...
    //customer submit reset password 
    $('#resetPass-form').submit(function (event) {
        event.preventDefault();
        var custResetPass = $('#custResetPass').val();
        var action = "resetPass";

        if (!custResetPass) {
            errorPrompt(append("the new password to reset."));

        } else {
            var customerInput = JSON.stringify({
                custResetPass: custResetPass,
                action: action
            });
