 ...
 * @author vinnie chin
 */
public class DalFo_GetBestSales extends Dal_Super<Product> {

    public DalFo_GetBestSales(Connection conn) {
        super(conn);
    }
    ...
    protected String sqlStatement() {
       ...
       return "SELECT "
                ...
                + "PRO.PRODUCTID "
                + "FROM PRODUCT PRO "
                + "JOIN PRODUCTDETAIL PD ON PD.PRODUCTID = PRO.PRODUCTID "
                + "WHERE PRO.STATUS = ? "
                + "GROUP BY PRO.PRODUCTNAME, PRO.PRODUCTID "
                + "HAVING SUM(PD.SALESOUTQTY) > 0 "
                + "ORDER BY SUM(PD.SALESOUTQTY) DESC "
